...
let audio = null;
let silenzio = null;   // elemento <audio> che tiene la sessione iOS in "playback"

// Su iPhone il solo Web Audio finisce nella categoria "ambient", che il tasto fisico
// Suoneria/Silenzioso zittisce: senza cuffie non si sentirebbe niente. Un elemento
// <audio> in riproduzione promuove la sessione a "playback", che il tasto non tocca.
// WAV mono, 8 kHz, 8 bit, un quarto di secondo di silenzio, in loop.
...
function sbloccaAudioIOS() {
  ...
  try {
    if (!silenzio) {
      silenzio = new Audio(SILENZIO_WAV);
      silenzio.loop = true;
      silenzio.volume = 0.01;   // non 'muted': un elemento muto non cambia la categoria
    }
    const p = silenzio.play();
...
function avviaAudio() {
  try {
    ...
    const c = new AC();
    const r = c.resume();       // Safari a volte consegna il contesto sospeso
